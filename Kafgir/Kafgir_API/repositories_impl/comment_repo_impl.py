...
class CommentRepositoryImpl(CommentRepository):
    ...
    def get_some_food_comments(self, food: Food, num: int) -> List[Comment]:
        comments = Comment.objects.filter(Q(food__id=food.id) , Q(confirmed=True) , ~Q(text=''))
    ...
    def get_food_comments(self, food: Food) -> List[Comment]:
        return list (Comment.objects.filter(Q(food__id=food.id) , Q(confirmed=True) , ~Q(text='')))

    def find_comment_by_user_id(self, id: int, food: Food) -> Comment:
        return Comment.objects.get(user=id,food__id=food.id)

    def are_there_any_user_comment(self, id: int, food: Food) -> bool:
        return Comment.objects.filter(user=id,food__id=food.id).exists()
